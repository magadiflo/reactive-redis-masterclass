...
Los usuarios que se conecten tarde a la sala, deben poder ver el historial de conversación, cosa que hasta este
momento no sucede.

## Solución a la tarea asignada - Chat History

En la implementación anterior, los usuarios que se conectaban tarde a una sala solo recibían los mensajes nuevos a
partir de su conexión. Todo lo anterior ya se había perdido, porque `Redis Pub/Sub` no guarda estado:
funciona en tiempo real, pero sin persistencia.

### Solución aplicada

Para resolverlo, se incorpora `Redis List (RListReactive)` como almacenamiento del historial de conversación.
...
- Los usuarios que ya estaban conectados siguen recibiendo los mensajes en tiempo real vía `Pub/Sub`.

### Detalles técnicos importantes

- `list.add(message).then(topic.publish(message))`. Encadena la acción de guardar el mensaje en Redis (lista
...
- `startWith(list.iterator())`. Cuando un cliente se conecta, primero se recorren todos los mensajes almacenados en
  Redis y se envían al cliente. Una vez enviado el historial, se engancha al flujo normal de `Pub/Sub` para recibir
  mensajes en tiempo real.


- `Persistencia en Redis`. Redis almacena la conversación en una lista bajo la clave `history:<room>`. Esto asegura que
  incluso si todos los usuarios se desconectan, el historial sigue ahí para futuras conexiones.
...
public class ChatRoomService implements WebSocketHandler {
    ...
    public Mono<Void> handle(WebSocketSession session) {
        ...
        // Agrega lista para almacenar historial de conversación
        RListReactive<String> list = this.client.getList("history:" + room, StringCodec.INSTANCE);

        //subscribe
        Mono<Void> incomingFlux = session.receive()
                .map(WebSocketMessage::getPayloadAsText)
                .flatMap(message -> list.add(message).then(topic.publish(message))) // Agregamos la conversación a la lista y publicamos el mensaje
                .doOnError(throwable -> log.error(throwable.getMessage()))
                .doFinally(signalType -> log.info("Subscriber finally {}", signalType))
                .then();
...
Supongamos que en la sala redis-course ya hubo una conversación previa entre Sam, Martín y Jake.
Si un nuevo usuario entra más tarde:

1. Primero verá en pantalla todo lo que se habló antes (gracias al `startWith(list.iterator())`).
2. A partir de ahí, seguirá recibiendo los mensajes nuevos como cualquier otro usuario.

De esta manera, la experiencia de usuario es más completa y consistente.
...
- La clave `history:redis-course` fue creada automáticamente al guardar mensajes en la sala `redis-course`.
- El `type` de la clave es `list`, lo que confirma que se está utilizando `RListReactive` como estructura subyacente.
...
- Los caracteres especiales (`\xc3\xa1`, `\xc3\xb3`, etc.) corresponden a `UTF-8` codificado en bytes. `Redis` no
  transforma tu JSON, simplemente lo almacena como texto plano. Cuando la aplicación lo lee, lo decodifica de vuelta a
  caracteres legibles (Martín, tardón, etc.).

Esta consulta a `Redis` confirma que:

- Los mensajes realmente están siendo persistidos.
- Están almacenados de manera ordenada (la lista mantiene el orden de llegada).
- El formato es legible y compatible con cualquier cliente que consuma el historial.

A continuación se muestra las imágenes de la conversación por chat.

![08.png](assets/section-08/08.png)

### 📊 Resumen de beneficios

- ✅ `Persistencia`: El historial se mantiene aunque nadie esté conectado.
- ✅ `Experiencia de usuario`: Los nuevos participantes no llegan a una "sala vacía".
- ✅ `Escalabilidad`: Redis maneja la persistencia y la distribución en múltiples instancias del backend.
