...
### Renombrando la implementación original

Como vamos a implementar una nueva versión del servicio con Redis, renombramos las clases que ya habíamos desarrollado
para mantener un orden claro entre ambas versiones:

````bash
ProductController -> ProductControllerV1
ProductServiceImpl -> ProductServiceImplV1
````

Con estos nombres podemos diferenciar fácilmente las versiones sin perder el historial de implementación.

### Preparando la nueva versión del servicio

Creamos una nueva clase de servicio `ProductServiceImplV2`, donde implementaremos la lógica de caché con `Redis` en la
siguiente lección. Por ahora solo declaramos la clase y los métodos que implementan la interfaz `ProductService`.
