...
### 🔹 ¿Qué es una List en Redis?

Una `List` en `Redis` es una estructura de datos que almacena una `colección ordenada de strings`. Internamente,
se implementa de forma muy eficiente, lo que la hace ideal para operaciones como `inserción/extracción` en los extremos.

### ✅ Características:

- Ordenada por inserción.
...
- `LRANGE users 0 1`: obtiene solo del índice 0 al 1.
...
- `LPOP`: elimina el primer elemento (inicio) → Comportamiento FIFO. (l=left)
...
- `RPOP`: elimina el último elemento (final) → Comportamiento LIFO. (r=right)
...
- Lista vacía: se ha eliminado todo.

### 📘 Comandos comunes para List
...
| `RPOP key`             | Elimina y retorna el último elemento         |
| `LRANGE key start end` | Retorna una porción de la lista (como slice) |
| `LLEN key`             | Retorna la cantidad de elementos             |
| `LTRIM key start end`  | Recorta la lista a un rango específico       |

### 💡 Notas adicionales

- Redis List es muy rápida para operaciones en los extremos (LPUSH, RPUSH, LPOP, RPOP) incluso con millones de
  elementos.
- Si no existe la clave y haces RPUSH o LPUSH, Redis crea automáticamente la lista.
- A diferencia de un array, acceder a un elemento en una posición intermedia o profunda de una lista en Redis puede ser
  lento, ya que Redis recorre la lista desde el inicio o el final hasta alcanzar el índice.

## 📦 Redis List como Stack (Pila)

Una `pila (stack)` es una estructura de datos basada en el principio `LIFO (Last-In, First-Out)`, es decir:
> El último elemento en entrar es el primero en salir.

`Redis` no tiene un tipo de dato específico `Stack`, pero podemos simular este comportamiento perfectamente usando la
estructura List.

### 🛠️ Cómo implementarlo en Redis

Para que una lista se comporte como pila:

- Usamos `RPUSH` para insertar elementos al final de la lista (lado derecho).
- Usamos `RPOP` para eliminar y retornar el último elemento (lado derecho).

Este patrón imita el comportamiento de una pila tradicional.

🔍 En este punto, la lista users contiene
...
📤 Eliminamos elementos como si fuera una pila (LIFO)
...
📌 Se eliminaron primero los últimos que se insertaron: "5" y "4".
...
- Este patrón es útil para funcionalidades como deshacer acciones, manejo de backtracking, evaluación de expresiones,
  etc.

## Redis Set

### 🧠 ¿Qué es un Set en Redis?

Un `Set` en `Redis` es una `colección desordenada` de valores únicos (tipo String).

- Es similar a un Set en Java:
- No permite duplicados.
- El orden de los elementos no está garantizado.
- Muy útil para validar pertenencia o manejar grupos sin repetición.
...
### ⚙️ Comandos clave con ejemplos

````bash
# Agregamos múltiples elementos al set
...
# Intentamos agregar un duplicado (no se añade)
127.0.0.1:6379> sadd roles guest
(integer) 0
````
...
- Redis Set es perfecto cuando necesitas almacenar elementos únicos sin importar el orden.
- Proporciona operaciones eficientes para añadir, eliminar y verificar pertenencia.
- Es altamente útil para manejar grupos dinámicos en aplicaciones web, como roles, etiquetas, sesiones activas, etc.

## 🔁 Redis Set – Intersección, Unión y Diferencia

### 📌 Recordatorio: ¿Qué es un Set en Redis?

Un Set es una colección de elementos únicos y desordenados. Redis permite realizar operaciones de conjunto similares a
las matemáticas de teoría de conjuntos, como:

- `SINTER` → Intersección
- `SUNION` → Unión
- `SDIFF` → Diferencia
- También existen variantes que almacenan el resultado: `SINTERSTORE`, `SUNIONSTORE`, `SDIFFSTORE`.
...
📌 El valor "4" es el único presente en los tres conjuntos.

### 🔹 Unión (SUNION)

Retorna todos los elementos únicos combinados de los sets.
...
Retorna los elementos del primer set que no están presentes en los demás.
...
📌 Aquí identificamos los usuarios de `skill:java` que no tienen antecedentes.

### 💾 Almacenar resultados: SINTERSTORE
...
✅ El resultado de la `intersección` se guarda en la nueva clave `java-js`:
...
- Estas operaciones son extremadamente útiles cuando se trabaja con agrupaciones de usuarios, etiquetas, roles,
  permisos, etc.
- Redis realiza estas operaciones en memoria de manera rápida y eficiente.
- SINTERSTORE, SUNIONSTORE, SDIFFSTORE permiten persistir los resultados de las operaciones.

## 🎯 Redis Sorted Set (ZSet)

### 📌 ¿Qué es un Sorted Set?

Un `Sorted Set (ZSet)` en Redis es una colección de elementos únicos, al igual que un Set tradicional, pero donde cada
elemento está asociado a un valor numérico llamado `score`.

- Los elementos están ordenados automáticamente por su score de forma ascendente.
- Ideal para `rankings`, `puntuaciones`, `sistemas de recomendación`, `popularidad de productos`, etc.
...
- Aquí insertamos productos con un score inicial de 0.

### 📈 Incrementar puntuación con ZINCRBY
...
- Aquí se simula que el producto iphone es más popular y sube su puntuación.
...
- Con `withscores`, también muestra la puntuación asociada.
...
`ZRANGE -1 -1`: obtiene el último elemento, es decir, el de mayor puntuación.
...
- Redis garantiza que los elementos estén ordenados automáticamente al insertarlos o modificar sus scores.
- Los scores pueden ser valores negativos, decimales o enteros.
- Dos elementos pueden tener el mismo score; en ese caso se ordenan lexicográficamente.

### 🛠 Casos de uso

- Ranking de usuarios por puntuación
- Lista de productos más visitados
- Noticias o posts más populares
- Control de sesiones por actividad reciente

## 🔁 Redis Transactions

Las `transacciones en Redis` permiten `agrupar varios comandos` que se ejecutarán secuencialmente como una
`unidad atómica`. Esto es útil cuando múltiples usuarios o procesos pueden modificar las mismas claves, y queremos
evitar inconsistencias o condiciones de carrera (`race conditions`).

### 🧠 Conceptos clave

- `WATCH`: Observa una o más claves por cambios. Si alguna de ellas cambia antes de ejecutar la transacción (`EXEC`),
  `Redis` cancelará la transacción.
- `MULTI`: Indica el inicio de una transacción. A partir de aquí, los comandos se encolan y no se ejecutan
  inmediatamente.
- `EXEC`: Ejecuta todos los comandos encolados dentro de MULTI. Si alguna clave observada con WATCH fue modificada, se
  aborta (retorna `nil`).
- `DISCARD`: Cancela la transacción y descarta todos los comandos encolados.
...
### 👤 Simulación de Usuario 1

Este usuario observa las claves `user:1:balance` y `user:2:balance`, y luego inicia una transacción para transferir
saldo.

El resultado `nil` significa que `otro cliente (usuario 2)` modificó alguna de las claves observadas, lo que invalidó la
transacción.
...
### 👤 Simulación de Usuario 2

Este usuario también observa las mismas claves y realiza la transacción. Como fue el primero en ejecutar `EXEC`, su
transacción se completa exitosamente.
...
Redis no tiene `rollback` automático como una base de datos relacional, pero puedes descartar manualmente una
transacción antes de ejecutarla con `DISCARD`.

Este comando cancela la transacción. Ninguno de los comandos encolados se ejecuta.
...
- Si quieres lógica más compleja con garantías ACID completas, considera usar Lua Scripts (EVAL), que también son
  atómicos.
