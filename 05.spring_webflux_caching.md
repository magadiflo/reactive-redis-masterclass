...
### ¿Cómo funciona `@CachePut`?

La anotación `@CachePut` no impide que el método se ejecute. Todo lo contrario:

> ✅ El método se ejecuta siempre y su resultado es almacenado (o actualizado) en caché, usando la `cacheName` y la
> `key` proporcionadas.

Este comportamiento es útil cuando:

- Quieres `mantener los datos de caché sincronizados` con el resultado de una operación.
- Necesitas que la ejecución del método se realice, pero también deseas actualizar el valor cacheado, como cuando hay
  una `actualización periódica`, una inserción o una modificación.

A continuación comparamos la anotación `@CachePut` con otras anotaciones clave del sistema de caché de Spring Framework.

### 🧩 Comparación de anotaciones de caché

| Anotación     | ¿Ejecuta el método?                      | ¿Consulta caché antes? | ¿Actualiza caché?     | ¿Elimina caché? |
|---------------|------------------------------------------|------------------------|-----------------------|-----------------|
| `@Cacheable`  | ❌ (si hay key en caché)<br>✅ (si no hay) | ✅                      | ✅ (si no había key)   | ❌               |
| `@CacheEvict` | ✅                                        | ❌                      | ❌                     | ✅               |
| `@CachePut`   | ✅                                        | ❌                      | ✅ (siempre actualiza) | ❌               |

- `@Cacheable`: Evita la ejecución del método si el valor ya está cacheado. Si no, lo ejecuta y guarda el resultado.
- `@CacheEvict`: Siempre ejecuta el método y elimina el valor cacheado (después de ejecutar el método).
- `@CachePut`: Siempre ejecuta el método y actualiza el valor en caché, sobrescribiendo lo anterior.

### ☁️ Ejemplo práctico con clima (`@CachePut`)

Vamos a simular un servicio que obtiene información meteorológica desde un cliente externo, y cachea sus resultados por
código postal (`zipCode`).

📦 Clase `ExternalServiceClient`
...
- El método `getWeatherInfo(int zipCode)` simula obtener datos externos (por ejemplo, temperatura).
- Se anota con `@CachePut`, lo que significa que siempre se ejecutará y actualizará la caché bajo la clave `weather`
  con la key `zipCode`.
- Se genera un número aleatorio entre 60 y 100 como valor simulado.

🌦️ Clase `WeatherService`
...
- El método `getInfo(int zipCode)` expone los datos cacheados usando `@Cacheable`, sin ejecutar lógica adicional.
    - Si la clave está presente, retorna su valor desde caché.
    - Si no está en caché, se retorna `0` (valor por defecto simulado).


- El método `update()` se `ejecuta automáticamente` cada `10 segundos` gracias a `@Scheduled(fixedRate = 10_000)`.
    - Este método llama a `getWeatherInfo(...)` del cliente externo con los códigos postales del 1 al 5.
    - Como `getWeatherInfo(...)` está anotado con `@CachePut`, siempre se ejecuta y actualiza la caché.

⚙️ Clase `WeatherController`
...
- `@EnableScheduling`: Habilita la ejecución programada de métodos (`@Scheduled`).

## 🧠 @CachePut - Part 2

Una vez que ejecutamos la aplicación, podemos observar cómo el método `getWeatherInfo(int zipCode)` de la clase
`ExternalServiceClient` es invocado automáticamente cada 10 segundos gracias al método `update()` de `WeatherService`,
el cual está anotado con `@Scheduled(fixedRate = 10_000)`.

Esto lo podemos comprobar con los siguientes logs de consola:

````bash
2025-08-01T11:18:45.805-05:00  INFO 15548 --- [redis-spring] [   scheduling-1] d.m.r.a.weather.service.WeatherService   : Actualizando clima
2025-08-01T11:18:45.805-05:00  INFO 15548 --- [redis-spring] [   scheduling-1] d.m.r.a.w.service.ExternalServiceClient  : zipCode: 1
2025-08-01T11:18:45.829-05:00  INFO 15548 --- [redis-spring] [   scheduling-1] d.m.r.a.w.service.ExternalServiceClient  : zipCode: 2
2025-08-01T11:18:45.831-05:00  INFO 15548 --- [redis-spring] [   scheduling-1] d.m.r.a.w.service.ExternalServiceClient  : zipCode: 3
2025-08-01T11:18:45.834-05:00  INFO 15548 --- [redis-spring] [   scheduling-1] d.m.r.a.w.service.ExternalServiceClient  : zipCode: 4
2025-08-01T11:18:45.837-05:00  INFO 15548 --- [redis-spring] [   scheduling-1] d.m.r.a.w.service.ExternalServiceClient  : zipCode: 5
2025-08-01T11:18:55.781-05:00  INFO 15548 --- [redis-spring] [   scheduling-1] d.m.r.a.weather.service.WeatherService   : Actualizando clima
2025-08-01T11:18:55.781-05:00  INFO 15548 --- [redis-spring] [   scheduling-1] d.m.r.a.w.service.ExternalServiceClient  : zipCode: 1
...
````

Dado que el método `getWeatherInfo(...)` está anotado con `@CachePut(cacheNames = "weather", key = "#zipCode")`, este
`se ejecuta siempre y actualiza el valor en caché` correspondiente al `zipCode`. En otras palabras, `Redis` mantiene los
valores actualizados con cada ejecución programada del método.

### Consultas desde el cliente

Cuando un cliente realiza una petición al método `getInfo(int zipCode)` del `WeatherService`, se aplica lo siguiente:
...
- Si existe un valor en la caché para el `zipCode` proporcionado, este se retorna inmediatamente desde `Redis`.
- Si `no existe`, se ejecuta el método y se retorna `0`.

> 🧠 Lo interesante aquí es que, debido a que `Redis` es constantemente actualizado por el `@Scheduled`, el método rara
> vez retorna el valor 0 en la práctica, siempre y cuando el `zipCode` consultado esté dentro del rango actualizado
> (1 al 5).
...
````bash
$ curl -v http://localhost:8080/api/v1/weathers/1 | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 2
...
Unos segundos después (tras una nueva actualización automática):
...
Como vemos, el valor varía porque el método `@CachePut` se ejecuta periódicamente, ya que es llamado por el método
`update()` del `WeatherService`, generando un nuevo valor aleatorio que es almacenado nuevamente en `Redis`.

### 📌 Observaciones clave

- `@CachePut` no impide que el método se ejecute, todo lo contrario: se ejecuta siempre.
- Se utiliza principalmente para actualizar valores en caché activamente, ideal para tareas programadas o escenarios
  donde los datos deben mantenerse frescos (ej. clima, precios, disponibilidad, etc.).
- Mientras tanto, `@Cacheable` sigue el patrón `Cache Aside`: `solo ejecuta el método si no hay valor en caché`.
- Gracias a esta combinación, los consumidores de `getInfo(...)` siempre reciben valores actualizados desde `Redis`,
  sin necesidad de recalcular o ejecutar lógica costosa.

## ⚠️ Limitaciones del uso de anotaciones de caché en Spring

Las anotaciones como @Cacheable, @CacheEvict o @CachePut son muy convenientes y fáciles de usar en entornos imperativos.
Sin embargo, presentan ciertas limitaciones importantes, sobre todo cuando trabajamos en un entorno reactivo con Spring
WebFlux y programación no bloqueante.

### 🚫 Principales limitaciones

1. `No soportan TTL directamente`. No es posible establecer un `time-to-live` (expiración) de forma declarativa usando
   solo las anotaciones de Spring.


2. `Falta de flexibilidad avanzada`. Las anotaciones no permiten realizar configuraciones más complejas, como lógica
   condicional personalizada para almacenar, serializar o invalidar datos en caché.


3. ❗`No funcionan con tipos Publisher (como Mono o Flux)`. Esta es una de las limitaciones más críticas en
   aplicaciones reactivas. El uso de anotaciones como `@Cacheable`
   `no es compatible con métodos que retornen valores reactivos`.

### 🧪 Ejemplo que ilustra el problema
...
public class ProductService {
    ...
    // ✅ Funciona correctamente con programación imperativa
    ...
    // ❌ No funciona correctamente con programación reactiva
    @Cacheable(cacheNames = "product", key = "#id")
...
### ✅ ¿Qué ocurre en el primer método?

- Es un enfoque `imperativo`: el método se ejecuta completamente, retorna un objeto concreto (`Product`) y ese objeto
  puede ser fácilmente cacheado por Spring.
- Spring puede interceptar el retorno, almacenarlo y reutilizarlo en futuras llamadas.

### ❌ ¿Qué ocurre en el segundo método?

- El método retorna un `Mono<Product>`, es decir, no retorna el objeto `Product` en sí, sino un `publisher` que
  `define una secuencia a ejecutarse en el futuro`.
- En programación reactiva, al ejecutar el método no se ejecuta la lógica de negocio como tal, sino que simplemente se
  construye el pipeline de operaciones que será ejecutado más adelante cuando alguien se suscriba (`subscribe()`). Por
  eso, en ese momento aún no existe el valor real que Spring necesita para almacenarlo en caché.
- Cuando Spring intercepta este método, `no tiene aún el valor real a cachear`, solo tiene la definición del pipeline
  reactivo.
- Por tanto, `no puede almacenar en caché el resultado real`, ya que este solo se conocerá cuando el Mono se suscriba
  y se complete.

### 💡 Conclusión

> Aunque las anotaciones de Spring para caché son poderosas y prácticas en aplicaciones tradicionales, no son adecuadas
> para trabajar con tipos reactivas como `Mono` o `Flux`.

Más adelante, veremos cómo resolver esta limitación implementando caching manual o utilizando soluciones que funcionen
bien con programación reactiva y Redis. Esto también nos dará un mayor control y flexibilidad, por ejemplo, para definir
TTL, usar operadores reactivos, manejar errores, condicionar el cacheo, etc.

## City Service - Client

La implementación de este servicio será aplicando el patrón `Caché Aside` que vimos en apartados iniciales de esta
documentación.

![02.png](assets/section-05/02.png)

Pero para nuestro caso, no usaremos una base de datos como fuente externa, sino más bien utilizaremos una api de un
servicio que veremos luego. Más o menos tendremos esta arquitectura:

![04.png](assets/section-05/04.png)

Primero empezamos levantando la aplicación que nos proporcionó el tutor. Esta aplicación será nuestro servicio
externo al que nos vamos a comunicar cuando no encontremos los datos en redis.

````bash
D:\programming\spring\01.udemy\03.vinoth_selvaraj\reactive-redis-masterclass (feature/section-5)
$ java -jar .\external\city-api\city-api.jar
...
2025-08-04 10:33:00.692  INFO 14220 --- [           main] c.v.weatherapi.CityApiApplication        : Starting CityApiApplication v0.0.1-SNAPSHOT using Java 21.0.6 on SysEngJava with PID 14220 (D:\programming\spring\01.udemy\03.vinoth_selvaraj\reactive-redis-masterclass\external\city-api\city-api.jar started by magadiflo in D:\programming\spring\01.udemy\03.vinoth_selvaraj\reactive-redis-masterclass)
2025-08-04 10:33:00.697  INFO 14220 --- [           main] c.v.weatherapi.CityApiApplication        : No active profile set, falling back to default profiles: default
2025-08-04 10:33:02.474  INFO 14220 --- [           main] o.s.b.web.embedded.netty.NettyWebServer  : Netty started on port 3030
2025-08-04 10:33:02.485  INFO 14220 --- [           main] c.v.weatherapi.CityApiApplication        : Started CityApiApplication in 2.297 seconds (JVM running for 2.775)
````

La siguiente petición nos permite probar el servicio externo. La respuesta es un objeto JSON con información geográfica,
demográfica y climática.

````bash
$ curl -v http://localhost:3030/open-city-api/10001 | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 156
...
- Se usa un record para representar de manera concisa e inmutable los datos que nos interesan.
- Aunque el JSON tiene más campos, solo extraemos aquellos que nos importan. `Spring` y `Jackson` manejan el mapeo
  ignorando los demás sin problema.

### ⚙️ Cliente Web con WebClient
...
- Se usa `@Value("${city.service.url}")` para inyectar la URL base desde el `application.yml` o
  `application.properties`. Esto permite que sea configurable.
- El `WebClient` se construye de manera programática usando `.builder()`. Podría considerarse hacer esto a través de un
  `@Bean`, pero en este caso es una decisión válida y encapsulada.
- El método `getCity(...)` realiza una petición `GET` al endpoint `/open-city-api/{zipCode}` y retorna un `Mono<City>`
  que representa de forma reactiva la respuesta.

## 🧩 Capa de Servicio Reactiva con Redisson
...
- `CityClient` es el servicio que usa `WebClient` para obtener datos desde un servicio externo (por ejemplo,
  `/open-city-api/{zipCode}`).
- `RedissonReactiveClient` permite trabajar con estructuras reactivas como `RMapReactive`.
- `getMap("city", codec)` obtiene un mapa `Redis` reactivo usando como `key` un `String (el zipCode)` y como value un
  `City`. El `TypedJsonJacksonCodec` se asegura de que los datos se serialicen y deserialicen correctamente.

### 🚦 Método principal: `getCity(...)`

1. Busca en caché
    ````bash
    this.cityMap.get(zipCode)
    ````
    - Si la key (el `zipCode`) ya está almacenada en `Redis`, retorna el `City` directamente como `Mono<City>`.
    - Si no existe, se emite un `Mono.empty()`.

2. Si no hay en caché (`switchIfEmpty`), buscar en fuente externa
    ````bash
    this.cityClient.getCity(zipCode)
    ````
    - Hace la llamada HTTP al servicio externo para recuperar los datos del `City`.

3. Guardar el resultado en caché (`fastPut`) y luego retornar
    ````bash
    .flatMap(city -> this.cityMap.fastPut(zipCode, city).thenReturn(city))
    ````
    - `fastPut()` guarda el `City` en el map `Redis` con el `zipCode` como `key`.
    - Luego retorna el mismo city para completar la respuesta.

### 🧠 Observaciones clave

- Este patrón es una `implementación manual del patrón` `Cache Aside`, pero en `entorno reactivo`.
- `Redis` se usa como caché de lectura: primero se intenta leer, y si no hay, se hace la llamada costosa y se almacena.
- `No hay anotaciones de caché involucradas` porque, como vimos antes, `no funcionan con Mono o Flux`.

### 🔍 Diferencia entre `fastPut` y `put` en Redisson

`Redisson` proporciona dos métodos principales para insertar valores en un `RMap`:

- `put(key, value)`
    - → Inserta o actualiza un valor y retorna el valor anterior asociado a la clave, si lo hay.
    - → Es útil si necesitas saber si había un valor anterior.


- `fastPut(key, value)`
    - → También inserta o actualiza un valor, pero no retorna nada, solo un `Mono<Boolean>` indicando si el valor fue
      insertado como nuevo (true) o si fue actualizado (false).
    - → Es más eficiente y rápido, ideal cuando no necesitas conocer el valor previo.

> ✅ En nuestro caso usamos `fastPut(...)` porque no nos interesa el valor anterior, solo queremos insertar en caché y
> devolver el valor nuevo.

## 🧩 Capa de Servicio Reactiva con Redisson – Versión con Logs (Trazabilidad)

En esta lección veremos una nueva versión del método `getCity(...)` de la clase `CityService`. Este método no solo
gestiona el flujo reactivo entre caché y fuente externa, sino que registra todo lo que ocurre en tiempo de ejecución,
lo cual es muy útil en aplicaciones productivas o durante el desarrollo/debugging.
...
1. `doOnNext(...)` tras `get(...)`:
    - Detecta y registra el cache HIT.
    - Esto ocurre solo si Redis tenía el valor para el `zipCode`.
2. `switchIfEmpty(...)` con `Mono.defer(...)`:
    - Permite ejecutar una lógica lazy solo si la caché está vacía.
    - El `log.info("Cache MISS...")` se ejecuta únicamente si no hubo valor en caché → cache MISS.
3. `doOnNext(...)` tras `fastPut(...)`:
    - Registra si Redis guardó el nuevo valor (`true`) o sobrescribió uno existente (`false`).
    - Buena práctica usar el result booleano para mensajes diferenciados.
4. `thenReturn(city)`:
    - Asegura que el `Mono<City>` resultante emita el objeto original, no el boolean del `fastPut(...)`.

## 🧩 Controlador de CityService

Con la capa del cliente (`CityClient`) y la `lógica de caché` en el servicio (`CityService`) ya definidas, ahora
creamos un endpoint que permita a los consumidores obtener información de una ciudad usando su código postal
(`zipCode`).
...
### ⚙️ Configuración del cliente externo

En el archivo `application.yml` se define la URL base para el servicio externo de ciudades:
...
Esta URL será usada por el `CityClient` para construir las peticiones mediante `WebClient`.

### 🔁 Flujo resumido

1. Cliente llama al endpoint `GET /api/v1/cities/{zipCode}`.
2. `CityService` busca en `Redis` (`RMapReactive.get(...)`) usando como key el `zipCode`.
3. Si no encuentra la ciudad, se realiza la llamada al servicio externo (`CityClient.getCity(...)`).
4. Se almacena el resultado en Redis usando `fastPut(...)`.
5. Finalmente, se retorna el resultado al cliente.

## ⚡️ Reactive - Cache Aside Demo

En esta lección demostramos el funcionamiento manual del patrón `Cache Aside` usando `programación reactiva` y `Redis`
como `caché`.

### 🔎 Estado inicial del servidor Redis

Antes de realizar cualquier petición, verificamos que Redis no tenga datos almacenados:
...
Como vemos, no hay ninguna clave presente.

### 📲 Primera petición al endpoint

````bash
$ curl -v http://localhost:8080/api/v1/cities/10001 | jq
>
< HTTP/1.1 200 OK
< Content-Type: application/json
< Content-Length: 73
...
- Como el valor no existía en Redis, el servicio lo obtuvo desde la fuente externa.
- Antes de retornar la respuesta al cliente, el resultado fue almacenado en Redis.

### 📜 Log correspondiente:

````bash
d.m.redis.app.city.service.CityService   : Cache MISS - Consultando servicio externo para zipCode: 10001
d.m.redis.app.city.client.CityClient     : Llamando al servicio externo para zipCode: 10001
d.m.redis.app.city.service.CityService   : Nuevo valor guardado en Redis para zipCode: 10001 
````

### 📲 Segunda petición (mismo zipCode)
...
Esta vez no se hizo llamada al servicio externo. El valor fue devuelto directamente desde la caché.

````bash
d.m.redis.app.city.service.CityService   : Cache HIT - Obteniendo desde Redis para zipCode: 10001 
````

### 🗄 Revisión en Redis

Si consultamos directamente en Redis, observamos la estructura almacenada:
...
1) "\"10001\""
2) "{\"city\":\"New York\",\"stateName\":\"New York\",\"temperature\":61,\"zip\":\"10001\"}"
127.0.0.1:6379>
````

El mapa city contiene un solo entry:

- `Key`: "10001"
- `Value`: representación JSON del objeto City.

### 🧠 Conclusión

Este es un ejemplo clásico del patrón `Cache Aside` (Manual) en un entorno reactivo:

- Si el valor existe en caché → se devuelve directamente.
- Si no está en caché → se consulta desde la fuente externa y se guarda manualmente en Redis.

➡️ Este patrón es muy útil cuando:

- No puedes usar las anotaciones de Spring (`@Cacheable`, `@CachePut`, etc.).
- Trabajas con `Mono`, `Flux` u otros publishers reactivos.

## 🧠 Configurando TTL (Time To Live) en Redis con Redisson Reactivo

### 🎯 Objetivo

> Implementar una `estrategia de caché` con `expiración automática` usando programación reactiva en Spring Boot con
> Redisson, siguiendo el patrón `Cache-Aside` y estableciendo un `TTL (Time To Live`) por entrada.

### Se cambiará el `RMapReactive` por el `RMapCacheReactive`, ¿por qué?

La diferencia clave está en el soporte para `TTL (Time To Live)`:

### 📌 RMapReactive

- Es una interfaz reactiva para trabajar con mapas en Redis usando Redisson.
- Permite almacenar pares `clave-valor` de forma reactiva.
- ❌ No permite definir TTL para entradas individuales del mapa.
- ❌ No elimina automáticamente los datos una vez pasado un tiempo.

### 📌 RMapCacheReactive

- Extiende `RMapReactive`, pero `agrega soporte para TTL` y `expiración automática`.
- ✅ Permite definir el tiempo de vida por entrada, por ejemplo:
  `this.cityMap.fastPut(zipCode, city, 10, TimeUnit.SECONDS);`
- ✅ `Redisson` se encarga de eliminar automáticamente las entradas expiradas en segundo plano.
- ✅ Ideal para estrategias de caché donde los datos deben refrescarse o invalidarse después de cierto tiempo.

### 🧩 ¿Por qué usar RMapCacheReactive en este caso?

En tu implementación estás siguiendo el patrón `Cache-Aside` de forma manual (porque usas programación reactiva),
lo cual implica:

- Buscar en caché (`Redis`) primero.
- Si no existe, consultar el servicio externo.
- Guardar en caché el resultado.

Al aplicar un `TTL de 10 segundos` al guardar el dato, logras que:

- Redis `elimine automáticamente` ese dato luego de 10 segundos.
- Las siguientes peticiones, si han pasado los 10 segundos, `vuelvan a consultar el servicio externo`.
- Esto garantiza que los datos se mantengan frescos.

### ⚙️ Cambios realizados

Para lograr esto, realizamos las siguientes modificaciones en el servicio `CityService`:

1. Reemplazo de `RMapReactive` por `RMapCacheReactive`
    - Anteriormente usábamos:
      ````bash
      private final RMapReactive<String, City> cityMap;
      this.cityMap = client.getMap("city", codec);
      ````
    - Ahora usamos:
      ````bash
      private final RMapCacheReactive<String, City> cityMap;
      this.cityMap = client.getMapCache("city", codec);
      ````
      > 🔎 `RMapCacheReactive` extiende `RMapReactive` y permite establecer `TTL` por entrada, mientras que
      > `RMapReactive` no.

2. Establecimiento del TTL. Al guardar el resultado en caché, usamos el método `fastPut(...)` con `TTL`:

````bash
this.cityMap.fastPut(zipCode, city, 10, TimeUnit.SECONDS) 
````

Esto configura el `TTL en 10 segundos`. Después de este tiempo, el dato se elimina automáticamente de Redis.

### 🔄 Implementación final con logs y TTL
...
1. `Consulta en Redis`. Se busca si ya existe un valor en caché para el `zipCode` solicitado.
2. `Cache HIT`. Si existe, se retorna directamente desde Redis y se muestra un log informativo.
3. `Cache MISS`. Si no existe, se realiza una consulta al servicio externo (usando `WebClient`).
4. `Guardar en Redis con TTL`. Una vez obtenido el valor, se guarda en Redis usando `fastPut(...)` y se establece un
   `TTL de 10 segundos`.

### 🧪 Ejecución y validación

Levantamos el servicio externo con el que estamos probando la aplicación anterior.
...
2025-08-05 10:10:33.998  INFO 1420 --- [           main] c.v.weatherapi.CityApiApplication        : Starting CityApiApplication v0.0.1-SNAPSHOT using Java 21.0.6 on SysEngJava with PID 1420 (D:\programming\spring\01.udemy\03.vinoth_selvaraj\reactive-redis-masterclass\external\city-api\city-api.jar started by magadiflo in D:\programming\spring\01.udemy\03.vinoth_selvaraj\reactive-redis-masterclass)
2025-08-05 10:10:34.003  INFO 1420 --- [           main] c.v.weatherapi.CityApiApplication        : No active profile set, falling back to default profiles: default
2025-08-05 10:10:35.887  INFO 1420 --- [           main] o.s.b.web.embedded.netty.NettyWebServer  : Netty started on port 3030
2025-08-05 10:10:35.900  INFO 1420 --- [           main] c.v.weatherapi.CityApiApplication        : Started CityApiApplication in 2.45 seconds (JVM running for 2.97) 
````

Primera petición desde el cliente
...
Al ser la primera vez, observamos que el registro no está en redis, así que se llama al servicio externo para buscarlo,
colocarlo en redis y retornarlo.
...
Segunda petición (dentro de los 10 segundos)
...
Como el registro ya fue almacenado en redis y aún no ha vencido los 10 segundos al realizar la segunda petición este
registro se obtiene desde la caché de redis.

````bash
d.m.redis.app.city.service.CityService   : Cache HIT - Obteniendo desde Redis para zipCode: 10001 
````

Observamos el servidor de redis y los distintos comandos ejecutados:
...
127.0.0.1:6379> keys *
1) "city"
2) "redisson__timeout__set:{city}"
3) "redisson__execute_task_once_latch:{city}"
127.0.0.1:6379> keys *
...
- Inicialmente, el servidor de redis estuvo vacío.
- Luego de la primera petición, el servidor muestra la `key` `city` que es nuestro mapa `hash` de datos real donde se
  guardan los `zipCodes` como campos clave dentro de dicho hash. Por ejemplo:
    ````bash
    127.0.0.1:6379> HGETALL city
    1) "\"10001\""
    2) "{\"zip\":\"10001\",\"city\":\"New York\",...}" 
    ````
  🕓 Esta entrada expira en 10 segundos, como lo habíamos definido.
- `redisson__timeout__set:{city}`, es una estructura interna que usa `Redisson` para rastrear los `TTLs` de las entradas
  dentro del `hash` `city`.
- `"redisson__execute_task_once_latch:{city}"`, esta clave es un mecanismo de sincronización interna que Redisson
  utiliza para coordinar tareas de expiración.
- Lo importante es que el campo `(zipCode)` asociado al `TTL` se elimina luego de los `10 segundos` definidos, y eso
  es lo que hemos confirmado.

## 🔁 Actualización Periódica de Datos con Redis Cache (Cache-Aside Strategy)

En esta sección vamos a modificar la arquitectura de nuestra aplicación para implementar una estrategia donde los datos
del servicio externo se actualicen periódicamente en Redis. De esta manera, cuando un cliente realiza una petición al
backend, la respuesta es inmediata, ya que los datos estarán disponibles directamente en caché.

### 🎯 Objetivo

Consultar el endpoint externo que retorna más de `33,000` ciudades y actualizar esos registros en Redis cada 10
segundos. Esto evita llamadas repetitivas al servicio externo por cada petición individual.

![05.png](assets/section-05/05.png)

### 🌐 Servicio Externo

El endpoint externo está alojado en el puerto `3030`. El resultado de esa llamada devuelve un array de objetos `City`.
Ejemplo:

````bash
$ curl -v http://localhost:3030/open-city-api | jq
>
< HTTP/1.1 200 OK
...
En el `CityClient` se añade un nuevo método llamado `getAllCities()` que retorna un `Flux<City>` con todos los
registros:
...
Luego, en la clase `CityService` modificamos la lógica del servicio para implementar la actualización periódica de
Redis.
...
public class CityService {
    ...
    public Mono<City> getCity(final String zipCode) {
        return this.cityMap.get(zipCode)
                // Esto es opcional, pero agrega resiliencia en caso Redis falle.
                .onErrorResume(throwable -> this.cityClient.getCity(zipCode));

    }

    // Method que actualiza periódicamente la información en redis
    @Scheduled(fixedRate = 10_000) // Ejecuta cada 10 segundos
    public void updateCity() {
        this.cityClient.getAllCities()
                ...
                // Necesario ya que este method se ejecuta por sí mismo y no forma parte de una cadena reactiva.
                .subscribe();
    }
}
````
...
1. Realizamos una petición a nuestro backend (Puerto 8080)
...
La respuesta es inmediata porque Redis ya está poblado con los datos de la ciudad correspondiente.

2. Verificamos la ejecución periódica en consola

````bash
2025-08-05T13:07:08.688-05:00  INFO 1476 --- [redis-spring] [   scheduling-1] d.m.redis.app.city.client.CityClient     : Llamando al servicio externo para obtener todas las ciudades
2025-08-05T13:07:18.696-05:00  INFO 1476 --- [redis-spring] [   scheduling-1] d.m.redis.app.city.client.CityClient     : Llamando al servicio externo para obtener todas las ciudades
2025-08-05T13:07:28.702-05:00  INFO 1476 --- [redis-spring] [   scheduling-1] d.m.redis.app.city.client.CityClient     : Llamando al servicio externo para obtener todas las ciudades
2025-08-05T13:07:38.700-05:00  INFO 1476 --- [redis-spring] [   scheduling-1] d.m.redis.app.city.client.CityClient     : Llamando al servicio externo para obtener todas las ciudades
2025-08-05T13:07:48.693-05:00  INFO 1476 --- [redis-spring] [   scheduling-1] d.m.redis.app.city.client.CityClient     : Llamando al servicio externo para obtener todas las ciudades
...
````

### 📌 Beneficios del Enfoque

- ✅ Mejora el rendimiento: los datos ya están cacheados cuando el cliente hace una petición.
- ✅ Reduce la carga sobre el servicio externo.
- ✅ Mejora la resiliencia: incluso si el servicio externo falla momentáneamente, Redis tiene los datos más recientes.
- ✅ Permite configurar TTL (si se desea usar `RMapCacheReactive`).

### 📁 Estructura General del Flujo

Separar ambos caminos:

- Uno es el flujo del cliente (GET) que busca en Redis y si falla va al servicio externo.
- El otro es el flujo en background (updateCity) que actualiza todo Redis cada cierto tiempo.
...
- `getCity()` debe apuntar a Redis, y si no encuentra datos, entonces a `CityClient.getCity(...)`.
- `updateCity()` corre de forma independiente, periódicamente.
- Ambas estrategias conviven: una para peticiones `on-demand`, otra para actualización `proactiva`.
