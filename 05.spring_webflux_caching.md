...
- Luego de la primera petición, el servidor muestra el `hash` `city` que es nuestro mapa de datos real donde se guardan
  los `zipCodes` como claves dentro de dicho hash. Por ejemplo:
    ````bash
    127.0.0.1:6379> HGETALL city
    1) "\"10001\""
...
- Lo importante es que la key `city` se elimine luego del `TTL` que definimos (`10 segundos`), y eso es lo que hemos
  confirmado.
