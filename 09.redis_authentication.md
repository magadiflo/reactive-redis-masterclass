# Sección 09: Redis Authentication
...
## Redisson – Configurando credenciales con ACL

En la lección anterior creamos el usuario `magadiflo` con contraseña `pass123` y le quitamos el permiso de ejecutar el
comando `GET (-get)`.

Hasta este punto, en los ejemplos anteriores nos conectábamos a `Redis` sin credenciales (usuario `default`, sin
contraseña). Ahora configuraremos `Redisson` para usar un usuario específico con `ACL`.

### Configuración en Redisson

En el proyecto `projects/redisson-playground`, dentro de la clase de configuración, agregamos las credenciales:
    ...
    public RedissonClient getClient() {
        if (Objects.isNull(this.redissonClient)) {
            ...
            config.useSingleServer()
                    .setAddress("redis://127.0.0.1:6379")   // conexión al servidor Redis
                    .setPassword("pass123")                 // usuario ACL
                    .setUsername("magadiflo");              // contraseña del usuario
...
- Si omites estas líneas, la conexión se hará con el usuario `default` sin contraseña.
- `redis://127.0.0.1:6379` → protocolo redis, host y puerto del servidor.
...
- El `SET` se ejecuta sin problema (no está restringido).
...
Ahora probemos con una estructura distinta: `HyperLogLog`. Este tipo de estructura no utiliza `GET`, por lo que debería
...
Vemos que se ejecuta sin problemas dado que aquí no se usa el comando get.
...
✅ El test funciona correctamente porque los comandos `PFADD` y `PFCOUNT` no están restringidos en la ACL del usuario.
